...
function beginRound() { state.active = true; ui.combat.hidden = false; ui.bottom.hidden = false; $('playerName').textContent = state.mode === 'pvp' ? `P${state.turn}` : 'YOU'; $('enemyName').textContent = state.mode === 'pvp' ? `P${state.turn === 1 ? 2 : 1}` : 'SENTINEL'; spawnEnemy(); state.timerId = setInterval(() => { state.time--; ui.timer.textContent = state.time; if (state.time <= 0) finish(); }, 1000); if (state.mode === 'ai') state.enemyId = setInterval(enemyAttack, 1900); }
function spawnEnemy() { const old = document.querySelector('.enemy'); if (old) old.remove(); const enemy = document.createElement('button'); enemy.className = 'enemy'; enemy.setAttribute('aria-label', 'Enemy target'); enemy.innerHTML = '<span class="label">LOCK TARGET</span>'; enemy.style.left = `${12 + Math.random() * 68}%`; enemy.style.top = `${22 + Math.random() * 50}%`; enemy.addEventListener('click', hitEnemy); ui.arena.append(enemy); }
function hitEnemy(e) { e.stopPropagation(); if (!state.active || state.charge < 12) return; state.shots++; state.hits++; state.charge -= 12; state.enemy = Math.max(0, state.enemy - (state.mode === 'ai' ? 17 : 10)); e.currentTarget.classList.add('hit'); ping(540, .06); updateHud(); showToast('DIRECT HIT', '#55f6ff'); setTimeout(() => { if (state.enemy <= 0) finish(true); else spawnEnemy(); }, 210); }
function fire() { if (!state.active || state.charge < 12) { showToast('RECHARGING', '#ffcf65'); return; } state.shots++; state.charge -= 12; updateHud(); ping(160, .04); }
function enemyAttack() { if (!state.active) return; state.player = Math.max(0, state.player - (6 + Math.floor(Math.random() * 8))); ui.flash.classList.add('show'); setTimeout(() => ui.flash.classList.remove('show'), 100); ping(90, .13); updateHud(); showToast('INCOMING!', '#ff7aad'); if (state.player <= 0) finish(false); }
